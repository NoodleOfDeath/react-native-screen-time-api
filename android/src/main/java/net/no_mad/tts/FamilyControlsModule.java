...
import android.speech.tts.ReactNativeScreenTimeAPI;
...
public class FamilyControlsModule extends ReactContextBaseJavaModule {

    private ReactNativeScreenTimeAPI tts;
    ...
    public FamilyControlsModule(ReactApplicationContext reactContext) {
        super(reactContext);
        ...
        tts = new ReactNativeScreenTimeAPI(getReactApplicationContext(), new ReactNativeScreenTimeAPI.OnInitListener() {
            @Override
            public void onInit(int status) {
                synchronized(initStatusPromises) {
                    ready = (status == ReactNativeScreenTimeAPI.SUCCESS) ? Boolean.TRUE : Boolean.FALSE;
    ...
    public String getName() {
        return "ReactNativeScreenTimeAPI";
    }
    ...
    public void speak(String utterance, ReadableMap params, Promise promise) {
        ...
        if(speakResult == ReactNativeScreenTimeAPI.SUCCESS) {
        ...
        if (Build.VERSION.SDK_INT >= 21) {
            try {
                for(Voice voice: tts.getVoices()) {
                    ...
                    voiceMap.putBoolean("notInstalled", voice.getFeatures().contains(ReactNativeScreenTimeAPI.Engine.KEY_FEATURE_NOT_INSTALLED));
    ...
    public void engines(Promise promise) {
        ...
        if (Build.VERSION.SDK_INT >= 14) {
            try {
                ...
                for(ReactNativeScreenTimeAPI.EngineInfo engine: tts.getEngines()) {
    ...
    public void stop(Promise promise) {
        ...
        int result = tts.stop();
        boolean resultValue = (result == ReactNativeScreenTimeAPI.SUCCESS) ? Boolean.TRUE : Boolean.FALSE;
    ...
    private void requestInstallData(Promise promise) {
        ...
        intent.setAction(ReactNativeScreenTimeAPI.Engine.ACTION_INSTALL_TTS_DATA);
        try {
            getCurrentActivity().startActivity(intent);
            promise.resolve("success");
    ...
    private int speak(String utterance, String utteranceId, ReadableMap inputParams) {
        ...
        if (Build.VERSION.SDK_INT >= 21) {
            ...
            params.putInt(ReactNativeScreenTimeAPI.Engine.KEY_PARAM_STREAM, audioStreamType);
            params.putFloat(ReactNativeScreenTimeAPI.Engine.KEY_PARAM_VOLUME, volume);
            params.putFloat(ReactNativeScreenTimeAPI.Engine.KEY_PARAM_PAN, pan);
            if (filename.length() > 0) {
                return tts.synthesizeToFile(utterance, params, destinationFile, utteranceId);
            } else {
                return tts.speak(utterance, ReactNativeScreenTimeAPI.QUEUE_ADD, params, utteranceId);
            }
        } else {
            HashMap<String, String> params = new HashMap();
            params.put(ReactNativeScreenTimeAPI.Engine.KEY_PARAM_UTTERANCE_ID, utteranceId);
            params.put(ReactNativeScreenTimeAPI.Engine.KEY_PARAM_STREAM, String.valueOf(audioStreamType));
            params.put(ReactNativeScreenTimeAPI.Engine.KEY_PARAM_VOLUME, String.valueOf(volume));
            params.put(ReactNativeScreenTimeAPI.Engine.KEY_PARAM_PAN, String.valueOf(pan));
            if (filename.length() > 0) {
                return tts.synthesizeToFile(utterance, params, destinationFile.getPath());
            } else {
                return tts.speak(utterance, ReactNativeScreenTimeAPI.QUEUE_ADD, params);
            }
        }
    }
